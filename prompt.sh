#   Change Prompt
    ...
    # Enhanced branch color function
    branch_color ()
    {
        ...
        then
            ...
            then
                if git diff --cached --quiet 2>/dev/null >&2; then
                    ...
                    color=${c_yellow} # Staged changes
                fi
            else
                color=${c_red}        # Unstaged changes
            fi
        else
            return 0
        fi
    ...
    node_prompt_info() {
        if command -v node >/dev/null 2>&1; then
            ...
            if [[ -n "$node_version" ]]; then
                printf '\x01%s\x02[%s]\x01%s\x02 ' "${c_blue}" "${node_version}" "${c_sgr0}"
            fi
        fi
    }
    ...
    docker_prompt_info() {
        if command -v docker >/dev/null 2>&1; then
            ...
            if [[ -n "$context" && "$context" != "default" ]]; then
                printf '\x01%s\x02[🐳%s]\x01%s\x02 ' "${c_bright_blue}" "${context}" "${c_sgr0}"
            fi
        fi
    }

    # Enhanced AWS profile display with better colors
    aws_prompt_info() {
        if [[ -n "$AWS_PROFILE" || -n "$AWS_REGION" ]]; then
            ...
            if [[ "$profile" == *"prod"* ]]; then
                printf '\x01%s%s\x02[%s]\x01%s\x02 ' "${c_bold}" "${c_red}" "${profile}" "${c_sgr0}"
            elif [[ "$profile" == *"dev"* ]]; then
                printf '\x01%s\x02[%s]\x01%s\x02 ' "${c_green}" "${profile}" "${c_sgr0}"
            elif [[ "$profile" == *"staging"* || "$profile" == *"stage"* ]]; then
                printf '\x01%s\x02[%s]\x01%s\x02 ' "${c_yellow}" "${profile}" "${c_sgr0}"
            else
                printf '\x01%s\x02[%s]\x01%s\x02 ' "${c_cyan}" "${profile}" "${c_sgr0}"
            fi
        fi
    }
    ...
    PS1='$(aws_prompt_info)$(node_prompt_info)\u@\[${c_red}\]\W\[${c_sgr0}\]\[$(branch_color)\]$(parse_git_branch)\[${c_sgr0}\]$ '
